...
<div id="complaint-reply">
    <form @submit.prevent="getReply" class="col s12">
        <div class="center">
            <h4> Complaint Answer </h4>
        </div>
        <div class="row">
            <span> NIM: </span>
        ...
        <div class="row">
            <span> Nama: </span>
            <v-text-field value="nama" v-model="nama" readonly></v-text-field>
        </div>
        <div class="row">
            <span> Ruangan: </span>
        ...
        <div class="row" v-if="status === 'open'">
            <div class = "input-field col s12">
                <span> Answer: </span>
                <v-text-field type="text" v-model="answer" placeholder="Answer here"></v-text-field>
                <button type="submit" class="btn"> Submit </button>
            </div>
        </div>
        <div class="row">
            <router-link v-bind:to="{name: 'view-complaint', params: {id: id_complaint}}" class="btn grey"> Back </router-link>
        </div>
        <v-spacer></v-spacer>
    </form>
</div>
            ...
            .then(querySnapshot => {
                querySnapshot.forEach(doc =>{
                    if(!doc.exists){
                        ...
                        this.id_reply = doc.data().messages[0].id_reply
                        this.reply = doc.data().messages[0].reply
                        this.to = doc.data().messages[0].to
                        this.from = doc.data().messages[0].from
